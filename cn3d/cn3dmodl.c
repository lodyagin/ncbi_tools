...
static EnumFieldAssocPtr renderAlist[4] =
    { all_render_alist, all_render_alist, all_render_alist,
        all_render_alist };
...
/*---------------------------------------------------------*/
void LIBCALLBACK Cn3DCheckMGTurnOnOffStatus(PFB pfbThis, Int4 iModel,
                                            Int4 iIndex, Pointer ptr)
{
    PMGD pmgdThis = NULL;
    PDNMG pdnmgThis = NULL;
...
/*---------------------------------------------------------*/
static void CheckSalsaDisplay(PMMD pmmdThis, Int4 iCountStruc,
                              Byte bVisible)
{
    Uint2 entityID, itemID;

    entityID = BioseqFindEntity(pmmdThis->pSeqId, &itemID);
    /* todo: send OBJ_BIOSEQ update here */
    ...
    while (pdnmmHead) {
        ...
        if (pmmdThis) {
            ...
            for (pdnmg = pmmdThis->pdnmgHead; pdnmg != NULL;
                 pdnmg = pdnmg->next) {pmgd = (PMGD) pdnmg->data.ptrvalue;
                pmgd->bVisible = 1;
            }
        }
      setout1:
...
/*  DDV_ColorExecute(Cn3D_ColorData.pDDVColorGlobal, NULL, NULL);*/

    /* Cn3D_RedrawProc(FALSE); why is this necessary -- lyg */
    ...
    if (pfbThis->bMe == AM_MMD) {
        ...
        while (pvnmoThis) {
            ...
            if (IsGraphNode((PFB) pmodThis->pvnContains->data.ptrvalue)) {
                if (pmmdThis ==
                    GetParentMol((PFB) pmodThis->pvnContains->data.
                                 ptrvalue)) {
                    pmodThis->bVisible = Cn3D_IsVisible(Cn3D_ColorData.pDDVColorGlobal,
                                   pmmdThis);
;
                }
            }
        ...
        while (pvnmoThis) {
            ...
            while (pvnThis) {
                ...
                if (Cn3D_DisplayHighlight)
                    pmodThis->bVisible = pmgdThis->bHighlighted;
                else
                    pmodThis->bVisible = pmgdThis->bVisible;
                if (pmodThis->bVisible == 1)
                    break;
        ...
        while (pdnmmHead) {
            ...
            if (pmmdThis) {
                if (pmmdThis->bWhat == AM_PROT
                    ...
                    if (bVisible !=
                        pmsdMaster->bVisible) CheckSalsaDisplay(pmmdThis,
                                                                iCount,
                                                                pmsdMaster->
                                                                bVisible);
                }
                    ...
                    while (pdnmmHead) {
                        ...
                        if (pmmdThis) {
                            if (pmmdThis->bWhat == AM_PROT
                                ...
                                if (bVisible !=
                                    pmsdSlave->
                                    bVisible) CheckSalsaDisplay(pmmdThis,
                                                                iCount,
                                                                pmsdSlave->
                                                                bVisible);
                            }
...
static void Cn3D_DisplayProc(ButtoN b)
{
        ...
        while (pdnmmHead) {
            ...
            if (pmmdThis) {
                ...
                for (pdnmg = pmmdThis->pdnmgHead; pdnmg != NULL;
                    ...
                    pmgd = (PMGD) pdnmg->data.ptrvalue;
                    iCountItem =
                        iCountItemGet(pmsdMaster, pmmdThis, pmgd,
                                      iCountStruc);
                    pmgd->bVisible =
                        GetItemStatus(Cn3D_lFeature, iCountItem);
                    if (pmgd->bReserved
                        && (pmgd->bReserved == pmsdMaster->bAligned)
                        && !Cn3D_fAlignOn) pmgd->bVisible = 0;
                    if (
                        (!(pmgd->bReserved)
                         || (pmgd->bReserved != pmsdMaster->bAligned))
                        && !Cn3D_fUnalignOn) pmgd->bVisible = 0;
            ...
            while (pdnmmHead) {
                ...
                if (pmmdThis) {
                    ...
                    for (pdnmg = pmmdThis->pdnmgHead; pdnmg != NULL;
                        ...
                        pmgd = (PMGD) pdnmg->data.ptrvalue;
                        iCountItem =
                            iCountItemGet(pmsdSlave, pmmdThis, pmgd,
                                          iCountStruc);
                        pmgd->bVisible =
                            GetItemStatus(Cn3D_lFeature, iCountItem);
                        if (pmgd->bReserved) {
                            if (
                                (*(pmgd->pbMasterReserved) ==
                                 *(pmsdSlave->pbAligned))
                                && !Cn3D_fAlignOn) pmgd->bVisible = 0;
                            if (
                                (*(pmgd->pbMasterReserved) !=
                        ...
                        if (!(pmgd->bReserved) && !Cn3D_fUnalignOn)
                            pmgd->bVisible = 0;
                    }
                }
    ...
    if (GetStatus(Cn3D_bHideHighlight)
        || GetStatus(Cn3D_bResidueDisplayReset))
            Cn3DCheckTurnOnOffStatus();
...
GrouP LIBCALL AlignControls(Nlm_GrouP prnt)
{
    ...
    Cn3D_bDisplayAlignedDomain =
        CheckBox(g, "Aligned Domain Only", Cn3D_ListAlignedDomainProc);
...
/*Cn3D_bDisplayApply = PushButton(g, "Apply", Cn3D_DisplayProc); */
    ...
    Cn3D_bDisplayHighlight =
        CheckBox(h, "Show Selected Only", Cn3D_DisplayHighlightProc);
    AlignObjects(ALIGN_LEFT, (HANDLE) ppt4,
                 (HANDLE) Cn3D_bDisplayHighlight,
                 (HANDLE) Cn3D_bHideHighlight,
                 (HANDLE) Cn3D_bResidueDisplayReset, NULL);
    SetStatus(Cn3D_bDisplayHighlight, FALSE);
...
/*---------------------------------------------------------*/
void LIBCALLBACK DoCleanSpecialFeatureWithMGD(PFB pfbThis, Int4 iModel,
                                              Int4 iIndex, Pointer ptr)
{
    PDNMG pdnmgThis = NULL;
    PMGD pmgdThis = NULL;
...
/*---------------------------------------------------------*/
void LIBCALLBACK DoDeHighlightWithMGD(PFB pfbThis, Int4 iModel,
                                      Int4 iIndex, Pointer ptr)
{
    PDNMG pdnmgThis = NULL;
    PMGD pmgdThis = NULL;
...
/*---------------------------------------------------------*/
void LIBCALLBACK DoCleanJustHLStatusWithMGD(PFB pfbThis, Int4 iModel,
                                            Int4 iIndex, Pointer ptr)
{
    PDNMG pdnmgThis = NULL;
    PMGD pmgdThis = NULL;
...
/*---------------------------------------------------------*/
void LIBCALLBACK DoTurnOnSpecialFeatureWithMGD(PFB pfbThis, Int4 iModel,
                                               Int4 iIndex, Pointer ptr)
{
    PDNMG pdnmgThis = NULL;
    PMGD pmgdThis = NULL;
...
/*---------------------------------------------------------*/
void LIBCALLBACK DoUnLinkSpecialFeatureWithMGD(PFB pfbThis, Int4 iModel,
                                               Int4 iIndex, Pointer ptr)
{
    PDNMG pdnmgThis = NULL;
    PMGD pmgdThis = NULL;
...
/*---------------------------------------------------------*/
void LIBCALLBACK DoLinkSpecialFeatureWithMGD(PFB pfbThis, Int4 iModel,
                                             Int4 iIndex, Pointer ptr)
{
    PDNMG pdnmgThis = NULL;
    PMGD pmgdThis = NULL;
    ...
    if (pmgdThis && pmgdThis->bHighlighted == 1) {
        if (pmgdThis->iUserDefinedFeature != 0) {
            if (GetStatus(bFeatureAdd)) {
                sfipThis =
                    GetThisSpecialAlgorRenderSet(pmgdThis->
                                                 iUserDefinedFeature);
                if (sfipThis != NULL)
                    sfipThis->iRes--;
                ...
                pmgdThis->iUserDefinedFeatureOld =
        ...
        sfipThis =
            GetThisSpecialAlgorRenderSet(pmgdThis->iUserDefinedFeature);
        if (sfipThis != NULL)
            sfipThis->iRes++;
...
/*---------------------------------------------------------*/
SpecialFeatureInfoPtr SpecialFeatureInfoFree(SpecialFeatureInfoPtr
                                             sfipThis)
{
    PRK prkThis = NULL;
    PARS parsThis = NULL;
...
static void ChangeSpecialRenderProc(void)
{
    ...
    for (i = 0; i < 4; i++) {
        if (GetEnumPopup
            (Cn3D_pupModelRenderStyle[i], renderAlist[i], &val)) {
            j = (Int2) val;
        } else {
            j = 1;
        }
            ...
            if (pmmdThis) {
                ...
                for (pdnmg = pmmdThis->pdnmgHead; pdnmg != NULL;
                    ...
                    if (pmgd->iUserDefinedFeature > 0) {
                        ...
                        sfpThis = sfp;
                        while (sfpThis) {
                            if (pmgd->iUserDefinedFeature ==
                                sfpThis->choice) {
                                pmgd->FeatureOn =
                                    GetItemStatus(Cn3D_lFeature3, iCount);
                                break;
                            }
                            sfpThis = sfpThis->next;
                            iCount++;
...
void Cn3D_ModelRedrawWrapper(ButtoN b)
{
    ...
    if (GetStatus(bFeatureEdit)) {
        ...
        iAddedFeature++;
        ValNodeAddPointer(&sfp, iAddedFeature,
                          (SpecialFeatureInfoPtr) sfip);
    ...
    if (GetStatus(bFeatureAdd) || GetStatus(bFeatureEdit)) {
        ...
        if (GetStatus(bFeatureAdd)) {
            ...
            Cn3D_CleanJustHLStatus();
            SetStatus(bFeatureAdd, FALSE);
        } else if (GetStatus(bFeatureEdit))
            SetStatus(bFeatureEdit, FALSE);
    ...
    if (FeatureOverwritten)
        RemoveInvalideUserDefinedFeatures();
...
GrouP LIBCALL ModelControls(Nlm_GrouP prnt)
{
    ...
    ppt12 =
        StaticPrompt(h5,
                     "Define Features: Use Mouse to Select Residues First",
...
/*StaticPrompt(g, "         ",0, stdLineHeight + 5, systemFont, 'l'); */
    ...
    ppt1 =
        StaticPrompt(h1, "    Set Render", 0, stdLineHeight + 5,
                     systemFont, 'c');
    ppt[0] =
...
/*AlignObjects (ALIGN_CENTER, (HANDLE) ppt1, (HANDLE) ppt [1], NULL); */
    ...
    ppt3 =
        StaticPrompt(h1, "Render", 0, stdLineHeight + 5, systemFont, 'c');
...
/*StaticPrompt(g, "",0,0,Nlm_systemFont,'l'); */
    ...
    ppt5 =
        StaticPrompt(h4, "    Set Label", 0, stdLineHeight + 5, systemFont,
                     'c');
    ppt0[0] =
        StaticPrompt(h4, "  Amino Acid", 0, popupMenuHeight, programFont,
                     'l');
    ppt0[1] =
        StaticPrompt(h4, "  Nucleic Acid", 0, popupMenuHeight, programFont,
                     'l');
/*AlignObjects (ALIGN_CENTER, (HANDLE) ppt5, (HANDLE) ppt0 [1], NULL);  */
    ...
    ppt13 =
        StaticPrompt(h7, "Edit Features:", 0, stdLineHeight + 5,
                     systemFont, 'c');
...
/*-------------------------------------------*/
BiostrucFeatureSetDescrPtr GetDescrForUserDefinedFeature(SpecialFeaturePtr
                                                         sfpThis)
{
...
/*-------------------------------------------*/
ValNodePtr GetBiostrucFeatureLocationForUserDefinedFeature(PDNMS pdnmsThis,
                                                           Int2 iFeature)
{
...
UserObjectPtr GetUserObjectForUserDefinedFeature(PARS pars)
{
    ...
    FillUserObjectField(tail_ufp, oip);
    rgb = Cn3d_PaletteRGB[pars->PBBColor];
    ...
    FillUserObjectField(tail_ufp, oip);
    rgb = Cn3d_PaletteRGB[pars->PResColor];
    sprintf(str, "%d %d %d", *rgb, *(rgb + 1), *(rgb + 2));
    tail_ufp->data.ptrvalue = StringSave(str);
    ...
    FillUserObjectField(tail_ufp, oip);
    rgb = Cn3d_PaletteRGB[pars->NTBBColor];
    sprintf(str, "%d %d %d", *rgb, *(rgb + 1), *(rgb + 2));
    tail_ufp->data.ptrvalue = StringSave(str);
    ...
    FillUserObjectField(tail_ufp, oip);
    rgb = Cn3d_PaletteRGB[pars->NTResColor];
    sprintf(str, "%d %d %d", *rgb, *(rgb + 1), *(rgb + 2));
    tail_ufp->data.ptrvalue = StringSave(str);
...
/*-------------------------------------------*/
ValNodePtr
GetBiostrucFeaturePropertyForUserDefinedFeature(SpecialFeaturePtr sfpThis)
{
    ValNodePtr ppp = NULL;
    SpecialFeatureInfoPtr sfipThis = NULL;
    PARS parsThis = NULL;
...
/*-------------------------------------------*/
BiostrucFeatureSetPtr GetBiostrucFeatureSetForUserDefinedFeature(PDNMS
                                                                 pdnmsThis,
                                                                 BiostrucPtr
                                                                 bsp,
                                                                 SpecialFeaturePtr
                                                                 ...
                                                                 iCount)
{
    ...
    bsfp->Location_location =
        GetBiostrucFeatureLocationForUserDefinedFeature(pdnmsThis,
                                                        sfpThis->choice);
    ...
    bsfp->Property_property =
        GetBiostrucFeaturePropertyForUserDefinedFeature(sfpThis);
...
PDNMS Cn3DAddUserDefinedFeature(PDNMS pdnmsThis)
{
    ...
    while (sfpThis) {
        bsfsp =
            GetBiostrucFeatureSetForUserDefinedFeature(pdnmsThis, bsp,
                                                       sfpThis, iCount);
        if (bsfsp) {
            if (!bsfsp_head) {
                bsfsp_head = bsfsp;
...
/*-------------------------------------*/
void Cn3DIndexUserDefinedFeatureForMGD(PMSD pmsdThis, ValNodePtr llp,
                                       Int2 iFeature, Boolean FeatureOn)
{
    PDNMM pdnmmHead = NULL;
    PMMD pmmdThis = NULL;
    PDNMG pdnmgThis = NULL;
...
Int2 GetCn3d_PaletteRGBIndex(Uint1Ptr rgb)
{
    ...
    for (iCount = 0; iCount < CN3D_COLOR_MAX; iCount++) {
        if (*rgb == Cn3d_PaletteRGB[iCount][0] && *(rgb + 1) == Cn3d_PaletteRGB[iCount][1]
...
/*pars = (PARS) MemNew((size_t)(sizeof(ARS))); */
    ...
    while (ufp) {
        ...
        if (StringCmp(oip->str, "Protein Backbone Representative") == 0) {
            ...
            rgb[0] = (Uint1) rgb_short[0];
            rgb[1] = (Uint1) rgb_short[1];
            rgb[2] = (Uint1) rgb_short[2];
...
void Cn3DIndexUserDefinedFeatureForMSD(PDNMS pdnmsThis)
{
    ...
    while (bsfsp) {
        ...
        while (descr) {
            if (descr->choice == 1) {
                if (StringCmp
                    (descr->data.ptrvalue, "User Defined Features") == 0) {
                    UserDefinedFeatureFound = TRUE;
        ...
        if (UserDefinedFeatureFound) {
            ...
            Cn3DIndexUserDefinedFeatureForMGD(pmsdThis, llp, bsfsp->id,
                                              sfipThis->On);
        }
    ...
    while (bsfsp) {
        descr = bsfsp->descr;
        if (descr->choice == 1) {
            if (StringCmp(descr->data.ptrvalue, "User Defined Features") ==
                0) {
                if (bsfsp_prev)
...
void Cn3DIndexUserDefinedFeature()
{
    ...
    iAddedFeature = 0;
    while (sfpThis) {
        if (sfpThis->choice > iAddedFeature)
            iAddedFeature = sfpThis->choice;
        sfpThis = sfpThis->next;
    }
