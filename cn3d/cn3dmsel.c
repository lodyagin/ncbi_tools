...
static void Cn3D_ModelPickProc(LisT l)
{
	...
	if ((pmldThis->iType == Model_type_ncbi_vector) && (VectorKeeper == FALSE))
	   {
...
GrouP LIBCALL Cn3D_ModelSelect(GrouP g,  Boolean KeepVectors)
{
    ...
    if (pmsdThis) pdnmlThis = pmsdThis->pdnmlModels;
    if (!pdnmlThis)
      {
         Cn3D_gModelSelect = NormalGroup(g, 0, 1, "Select Models:",  systemFont, NULL);
         SetGroupMargins(Cn3D_gModelSelect, 10, 10);
         SetGroupSpacing(Cn3D_gModelSelect, 10, 5);
   	 StaticPrompt(Cn3D_gModelSelect, "No Models  ", 0, 0, systemFont, 'l'); 
	...
	{
	     pmldThis = (PMLD) pdnmlThis->data.ptrvalue;
	     if (!((pmldThis->iType == Model_type_ncbi_vector) && (KeepVectors == FALSE)))
		{
		    iCount++;
		    pcModelString[0]='\0';
		   ...
		   ValNodeCopyStr(&pvnModelStrings, pdnmlThis->choice, pcModelString); 
		}
	    pdnmlThis = pdnmlThis->next;
	} 
    ...
    if (pvnModelStrings) ValNodeFreeData(pvnModelStrings);
    iCount = 0;
    pdnmlThis = pmsdThis->pdnmlModels;
    while (pdnmlThis)
	{    /* walk models, set select status for list */
	     pmldThis = (PMLD) pdnmlThis->data.ptrvalue;
	     if (!((pmldThis->iType == Model_type_ncbi_vector) && (KeepVectors == FALSE)))
		{
		  iCount++;
...
Int2Ptr LIBCALL PickedModels(Int4Ptr piNumModels)
{
	...
	if (pmldThis->bSelected & (Byte) 0x01)
	  {
	    ValNodeAddInt(&pvnModelNos, pdnmlThis->choice, pdnmlThis->choice);
	    iCount++;
	  }
	 pdnmlThis = pdnmlThis->next;
     }
    ...
    if (iCount) i2Vec = I2Vector(0, iCount);
    else
      {
        return NULL;
      }
    ...
    if (pvnModelNos) ValNodeFree(pvnModelNos); 
    *piNumModels = iCount; 
    return i2Vec;   
}
