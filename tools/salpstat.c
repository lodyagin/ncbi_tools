              ...
              if(is_gap) {
                  ...
                  for(j =0; j<len; ++j) {
                      ...
                      if(res_1 == res_2) {
                          ...
                          ++mismatches;
                          if(code == Seq_code_ncbi4na && (res_1&res_2))
                              pmismatches++;
                      }
                  }
              }
...
NLM_EXTERN int LIBCALLBACK SortEarliestSeqAlign(VoidPtr a, VoidPtr b){
  SeqAlignPtr sa;
  SeqAlignPtr sb;
  DenseSegPtr dsp;
  Int4 j,ipos;
  Int4 start_posa,start_posb,a_order;
  ...
  start_posa=0;
  start_posb=0;
  a_order=0; /* XXX This is not very error tolerant.. but it will work */
  if(sa->segtype==2) {
        dsp = (DenseSegPtr) sa->segs;
	if(dsp!=NULL) {
	  for(j = 0; j<dsp->numseg; ++j) {
	    ipos = 2*j + a_order;
	    if((start_posa=dsp->starts[ipos])>=0) {
	      break;
	    }
	  }
	}
  ...
  if(sb->segtype==2) {
        dsp = (DenseSegPtr) sb->segs;
	if(dsp!=NULL) {
	  for(j = 0; j<dsp->numseg; ++j) {
	    ipos = 2*j + a_order;
	    if((start_posb=dsp->starts[ipos])>=0) {
	      break;
	    }
	  }
	}
  ...
  if(start_posa<  start_posb) {
    return (int)-1;
  } else if (start_posa >start_posb  ) {
    return (int)1;
  } else {
    return (int)0;
  }
...
NLM_EXTERN SeqAlignPtr LIBCALL SeqAlignNonOverlap(SeqAlignPtr salp,BioseqPtr query_bsp,BioseqPtr subject_bsp,SeqAlignPtr PNTR salp_leftover_ptr) {
    SeqAlignPtr salptmp;
    ...
    for (salptmp=salp; salptmp!=NULL; salptmp=salptmp->next) {
        ...
        if(SeqAlignStartStop(&lcl_a_start, &lcl_a_stop, &lcl_b_start, &lcl_b_stop,
            ...
            if(lcl_a_start<a_start)
                a_start=lcl_a_start;
            if(lcl_b_start<b_start)
                b_start=lcl_b_start;
    ...
    if(a_stop==-1 || b_stop ==-1) {
        ErrPostEx(SEV_WARNING,0,0,"SeqAlignSortNonOverlapPair: SeqAlign with no sequence overlap\n");
        return salp;
    }
            ...
            if(align->segtype==1) {
                ...
                while(ddp!=NULL && AcceptList[i]!=0) {
                    ...
                    for(k=qpos,l=spos;k<qend_pos && l<send_pos;k++,l++) {
                        if((qpos!=-1 && HitsPerQBase[k]>=Cov)  || (spos!=-1 && HitsPerSBase[l]>=Cov)) {
                            AcceptList[i]=0;
                            qpos+=qend_pos;
                        }
                    }
    ...
    if(new_salp!=NULL && new_salp->next !=NULL ) {
        for (j=0,salptmp=new_salp; salptmp!=NULL; j++,salptmp=salptmp->next) {
            salp_list[j]=salptmp;
        }
        n_sap=j;
        HeapSort((void *)salp_list,(size_t)n_sap,(size_t)sizeof(salp_list[0]),SortEarliestSeqAlign);
        new_salp=salptmp=salp_list[0];
        for(i=1;i<n_sap;i++) {
            if(salptmp==NULL) break;
            salptmp->next=salp_list[i];
            salptmp=salptmp->next;
        }
        if(salptmp!=NULL) salptmp->next=NULL;
    }
    Free(salp_list);
    return new_salp;
}
...
NLM_EXTERN SeqAlignPtr LIBCALL SeqAlignNonRedundant(SeqAlignPtr salp,BioseqPtr query_bsp,BioseqPtr subject_bsp,SeqAlignPtr PNTR salp_leftover_ptr) {
    ...
    salp = SeqAlignNonOverlap(salp,query_bsp,subject_bsp,&salp_leftover);
    if(!salp_leftover)
        return salp;
...
NLM_EXTERN SeqAlignPtr LIBCALL SeqAlignMergePairs(SeqAlignPtr salp,BioseqPtr query_bsp,BioseqPtr subject_bsp) {
  SeqAlignPtr salptmp;
  Uint1Ptr HitsPerQBase;
  Uint1Ptr HitsPerSBase;
  Uint1Ptr AcceptList;
  ...
  for (salptmp=salp; salptmp!=NULL; salptmp=salptmp->next) {
      Int4 lcl_a_start,lcl_a_stop,lcl_b_start,lcl_b_stop;
      SeqAlignStartStop(&lcl_a_start, &lcl_a_stop, &lcl_b_start, &lcl_b_stop, 
                        salptmp, query_bsp, subject_bsp);
      if(lcl_a_start!=-1 && lcl_a_start<a_start)
          a_start=lcl_a_start;
      if(lcl_b_start!=-1 && lcl_b_start<b_start)
          b_start=lcl_b_start;
      if(lcl_b_start!=-1 && lcl_a_stop>a_stop)
          a_stop=lcl_a_stop;
      if(lcl_b_stop!=-1 && lcl_b_stop>b_stop)
          b_stop=lcl_b_stop;
  }
  if(a_stop==-1 || b_stop ==-1 || a_start == INT4_MAX || b_start == INT4_MAX) {
      ErrPostEx(SEV_WARNING,0,0,"SeqAlignSortNonOverlapPair: SeqAlign with no sequence overlap\n");
      return salp;
  }
  ...
  for(i=0;i<n_sap;i++) {
	...
	while(ddp!=NULL && AcceptList[i]) {
	  qpos=ddp->starts[a_order];
	  if(qpos>=0) {
	    if(ddp->strands && best_qstrand != (Int2)ddp->strands[a_order]) {
	      AcceptList[i]=0;
	      break;
	    }
	  }
	  spos=ddp->starts[b_order];
	  if(spos>=0) {
	    if(ddp->strands && best_sstrand != (Int2)ddp->strands[b_order]) {
	      AcceptList[i]=0;
	      break;
	    }
	  }
...
NLM_EXTERN Int4 LIBCALL SeqAlignListCountMismatches(SeqAlignPtr sap,BioseqPtr query_bsp, 
  ...
  for(c_align = sap; c_align != NULL; c_align = c_align->next) {
	  ...
	  continue;
      } else if (!SeqAlignSimpleStats(c_align, query_bsp, subject_bsp, 
				&no_gap, &gap,
				&this_alength, 
...
NLM_EXTERN Int4 LIBCALL SeqAlignOverlapLenByLoc(SeqAlignPtr sap,SeqLocPtr query_slp,SeqLocPtr subject_slp) {
  ...
  Int4 maxlen=-1;
  Int4 diag;
  Int4 lens,lenq,qmax,smax,qmin,smin;
...
NLM_EXTERN FloatHi PNTR LIBCALL SeqAlignWindowScoreFN(SeqAlignPtr align, BioseqPtr bsp1, BioseqPtr bsp2,Int4 win_size, Int4 win_offset,Int4 win_step,Int4Ptr PNTR matrix,Int4 match, Int4 g_open, Int4 g_ext, Int4Ptr nwin,Int4Ptr perfect_length,Int4Ptr perfect_position) {
  ...
  if (align!=NULL) {
      ...
      if(align->segtype==2) {
          dsp=(DenseSegPtr) align->segs;
          strand = dsp->strands;          
          spp_1 = SeqPortNew(bsp1, 0, -1, strand==NULL ? 0 : strand[a_order], code1);
          spp_2 = SeqPortNew(bsp2, 0, -1, strand==NULL ? 0 : strand[b_order], code2);
          last_pos = 0;
          perfect=0;
          for(i =0; i<dsp->numseg; ++i) {
              Int4 ipos,jpos;
...
NLM_EXTERN SalpStatsResultsPtr LIBCALL SeqAlignStats(SeqAlignPtr salp_in, SalpStatsInputBlockPtr param,BioseqPtr query_bsp,BioseqPtr subject_bsp) {
    ...
    if(param->Mismatch) {
        SeqAlignCountMismatches(salp,query_bsp, 
		     subject_bsp,
    ...
    if(query_bsp->mol ==Seq_mol_aa)
        ...
        results->sub_reversed_strand = (SeqAlignStrand(salp,0)!=SeqAlignStrand(salp,1));
