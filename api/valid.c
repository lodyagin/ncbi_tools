			...
			if (sap->idx.parenttype == OBJ_BIOSEQSET) {
				...
				if (bsp->idx.parenttype == OBJ_BIOSEQSET) {
					...
					while (parent != NULL) {
						...
						if (parent->idx.parenttype != OBJ_BIOSEQSET) return;
	...
	while (sep != NULL)
	{
		...
		if (first) {
			...
			if (num_misplaced_features > 1) {
				ValidErr (vsp, SEV_REJECT, ERR_SEQ_PKG_FeaturePackagingProblem,
							"There are %d mispackaged features in this record.",
							(int) num_misplaced_features);
			} else if (num_misplaced_features == 1) {
				ValidErr (vsp, SEV_REJECT, ERR_SEQ_PKG_FeaturePackagingProblem,
							"There is %d mispackaged feature in this record.",
							(int) num_misplaced_features);
			}
      		first = FALSE;
      	}
        	...
        	if (bsp != NULL) {
        		if (mol == 0 || mol == Seq_mol_other) {
        			...
        			if (ISA_na (bsp->mol) != ISA_na (mol)) {
        			    ValidErr(vsp, SEV_REJECT, ERR_SEQ_PKG_SegSetMixedBioseqs,
        ...
        if (IS_Bioseq (sep)) {
        	...
        	if (bsp != NULL) {
        		if (mol == 0 || mol == Seq_mol_other) {
        			...
        			if (ISA_na (bsp->mol) != ISA_na (mol)) {
        			    ValidErr(vsp, SEV_REJECT, ERR_SEQ_PKG_PartsSetMixedBioseqs,
...

{
  ...
  if (sdp != NULL) {
    ...
    if (mip != NULL) {
      if (mip->completeness == 4 || mip->completeness == 5) return TRUE;
    }
  }
  return FALSE;
}
...
static void ValidateBioseqInst (GatherContextPtr gcp)
{
		...
		switch (sip1->choice) {
			...
			case SEQID_OTHER :
				...
				if (tsip != NULL && tsip->name != NULL) {
					...
					if (multitoken) {
						ValidErr (vsp, SEV_ERROR, ERR_SEQ_INST_SeqIdNameHasSpace,
								"Seq-id.name '%s' should be a single word without any spaces", tsip->name);
					}
				}
				break;
			case SEQID_PATENT :
	...
	for (sip1 = bsp->id; sip1 != NULL; sip1 = sip1->next) {
		...
		if (bsp2 == NULL) {
			if (! isPatent) {
				SeqIdWrite (sip1, buf1, PRINTID_FASTA_SHORT, 40);
				ValidErr (vsp, SEV_ERROR, ERR_SEQ_INST_IdOnMultipleBioseqs,
						"BioseqFind (%s) unable to find itself - possible internal error", buf1);
			}
		} else if (bsp2 != bsp) {
			SeqIdWrite (sip1, buf1, PRINTID_FASTA_SHORT, 40);
			ValidErr (vsp, SEV_ERROR, ERR_SEQ_INST_IdOnMultipleBioseqs,
					"SeqID %s is present on multiple Bioseqs in record", buf1);
		}
	}
	...
	if ((bsp->repr == Seq_repr_raw) || (bsp->repr == Seq_repr_const))
	{
		...
		if (len > len2)
		{
			ValidErr(vsp, SEV_REJECT, ERR_SEQ_INST_SeqDataLenWrong,"Bioseq.seq_data too short [%ld] for given length [%ld]",	(long)(len2 * divisor), (long)bsp->length);
			return;
		}
		else if (len < len2)
		{
			ValidErr(vsp, SEV_REJECT, ERR_SEQ_INST_SeqDataLenWrong,"Bioseq.seq_data is larger [%ld] than given length [%ld]",(long)(len2 * divisor), (long)bsp->length);
		}
				...
				switch (vnp->choice)
				{
					...
					case 2:     /* SeqLitPtr */
						...
						if (slitp->seq_data != NULL)
						{
							...
							if (sctp == NULL)
							{
                                ValidErr(vsp, SEV_REJECT, ERR_SEQ_INST_InvalidAlphabet, "Using illegal sequence alphabet [%d] in SeqLitPtr",
...

{
  ...
  if (StringICmp (dbt1->db, dbt2->db) != 0) return TRUE;
  oip1 = dbt1->tag;
  oip2 = dbt2->tag;
...
static Boolean GPSorNT (SeqEntryPtr sep, SeqLocPtr location)
...
static Boolean NGorNT (SeqEntryPtr sep, SeqLocPtr location)
...

{
  if (sfp != NULL && sfp->excpt) {
    if (StringICmp (sfp->except_text, "alternative processing") == 0 ||
        StringICmp (sfp->except_text, "alternate processing") == 0) return FALSE;
  }
  if (last != NULL && last->excpt) {
    if (StringICmp (last->except_text, "alternative processing") == 0 ||
        StringICmp (last->except_text, "alternate processing") == 0) return FALSE;
  }
  return TRUE;
}
		...
		if (last != NULL) {
				...
				if (fcontext.strand == strand ||
					...
					if (ivalssame && /* StringICmp (fcontext.label, label) == 0 && */
						...
						if (featdeftype == FEATDEF_PUB ||
							featdeftype == FEATDEF_REGION ||
						        featdeftype == FEATDEF_misc_feature ||
							featdeftype == FEATDEF_STS ||
							featdeftype == FEATDEF_variation) {
							severity = SEV_WARNING;
						} else if (StringICmp (fcontext.label, label) != 0 ||
							StringICmp (sfp->comment, comment) != 0) {
							if (! GPSorNT (vsp->sep, sfp->location)) {
								severity = SEV_WARNING;
							}
						}
			...
			if (fcontext.featdeftype == FEATDEF_mat_peptide_aa ||
			  ...
			  if (featdeftype == FEATDEF_mat_peptide_aa ||
			      featdeftype == FEATDEF_sig_peptide_aa ||
			      featdeftype == FEATDEF_transit_peptide_aa) {
...
static void ValidateBioseqContext (GatherContextPtr gcp)
{
	...
	for (sip = bsp->id; sip != NULL; sip = sip->next) {
		if (sip->choice == SEQID_PATENT) {
			...
			if (psip != NULL) {
				ipp = psip->cit;
				if (ipp != NULL && StringICmp (ipp->country, "US") == 0) return;
			}
			return;
		} else if (sip->choice == SEQID_PDB) {
			isPDB = TRUE;
    ...
    if (val == -1) {
      ...
      if (gbqual->val != NULL) {
        if (val == GBQUAL_rpt_type) {
          ...
          } else if (StringLen (gbqual->val) > 48) {
            ValidErr (vsp, SEV_ERROR, ERR_SEQ_FEAT_InvalidQualifierValue, "Illegal value for qualifier %s", gbqual->qual);
          }
        }
      }
...

{
  ...
  while (bssp != NULL &&
         ...
         bssp->idx.parenttype == OBJ_BIOSEQSET) {
...

{
  ...
  if (nuc == NULL || prot == NULL) return FALSE;
  bssp = GetParentNPS (nuc);
  if (bssp == NULL) return TRUE;
  if (GetParentNPS (prot) != bssp) return TRUE;
  return FALSE;
}
...

{
  ...
  if (nuc != NULL) {
    ...
    if (NucAndProtNotInNPS (nuc, bsp) && (! is_nt)) {
      ValidErr (vsp, SEV_ERROR, ERR_SEQ_FEAT_CDSproductPackagingProblem,
...

{
  ...
  if (sfp->data.choice == SEQFEAT_CDREGION) {
    ValidErr (vsp, sev, ERR_SEQ_FEAT_CDSgeneRange,
              "gene overlaps CDS but does not completely contain it");
  } else if (sfp->data.choice == SEQFEAT_RNA) {
    ValidErr (vsp, sev, ERR_SEQ_FEAT_mRNAgeneRange,
...

{
  ...
  ValidErr (vsp, sev, ERR_SEQ_FEAT_CDSmRNArange,
...

{
  ...
  if (bothstrands) {
    ValidErr (vsp, SEV_ERROR, ERR_SEQ_FEAT_BothStrands,
              "mRNA or CDS may not be on both strands");
  }
}
...

{
  ...
  if (pdseq != NULL) {
    ...
    if (mlen != plen) {
      ValidErr (vsp, SEV_ERROR, ERR_SEQ_FEAT_TranscriptLen,
                "Transcript length [%ld] does not match product length [%ld]",
                (long) mlen, (long) plen);
      ...
      ValidErr (vsp, SEV_ERROR, ERR_SEQ_FEAT_TranscriptMismatches,
                "There are %ld mismatches out of %ld bases between the transcript and product sequence",
...
SEQLOC_STOP);
	...
	{
		if (prot2len > 6) {
			if (! NGorNT (vsp->sep, sfp->location)) {
				sev = SEV_ERROR;
...
NLM_EXTERN void ValidateSeqLoc(ValidStructPtr vsp, SeqLocPtr slp, CharPtr prefix)
{
			...
			case SEQLOC_INT:
				...
				if (prevsip != NULL) {
				  if (SeqIdForSameBioseq(prevsip->id, sip2->id)) {
				    if (prevsip->strand == sip2->strand &&
				        prevsip->from == sip2->from &&
				        prevsip->to == sip2->to) {
	...
	if (vsp->sfp != NULL) {
		...
		if (sfp->excpt) {
		  ...
		  if (StringStr (sfp->except_text, "trans splicing") != NULL ||
	...
	if (vsp->sfp != NULL) {
		...
		if (vsp->sfp->idx.subtype == FEATDEF_PUB ||
	  		...
	  		vsp->sfp->data.choice == FEATDEF_misc_recomb ||
	  		vsp->sfp->data.choice == FEATDEF_primer_bind) return;
	}

	/* newer check for intervals out of order on segmented bioseq */

	bsp = BioseqFindFromSeqLoc (slp);
	if (bsp != NULL && SeqLocBadSortOrder (bsp, slp)) {
		ctmp = SeqLocPrint (slp);
		ValidErr (vsp, SEV_ERROR, ERR_SEQ_FEAT_SeqLocOrder, "%s: Intervals out of order in SeqLoc [%s]", prefix, ctmp);
		MemFree (ctmp);
	}
...

{
  ...
  for (vnp = head, index = 1; vnp != NULL; vnp = vnp->next, index++) {
    ...
    if (bs != NULL) {
      bslen = BSLen (bs);
      if (sgp->numval != bslen) {
        ValidErr (vsp, SEV_ERROR, ERR_SEQ_GRAPH_GraphByteLen, "SeqGraph (%ld) and ByteStore (%ld) length mismatch",
                  (long) sgp->numval, (long) bslen);
      }
    }
  }
  if (outOfOrder) {
  ...
  if (seqlen != gphlen && bsp->length != gphlen) {
    gcp->itemID = firstsgitemid;
    ValidErr (vsp, SEV_ERROR, ERR_SEQ_GRAPH_GraphBioseqLen, "SeqGraph (%ld) and Bioseq (%ld) length mismatch",
              (long) gphlen, (long) seqlen);
  }
  ...
  if (bsp->repr == Seq_repr_delta) {
    if (head != NULL && head->next != NULL) {
          ...
          case 1 :
            ...
            if (slocp != NULL && slocp->choice != SEQLOC_NULL) {
              ...
              if (sgp->numval != slplen) {
                gcp->itemID = sgp->idx.itemID;
                ValidErr (vsp, SEV_WARNING, ERR_SEQ_GRAPH_GraphSeqLocLen, "SeqGraph (%ld) and SeqLoc (%ld) length mismatch",
                          (long) sgp->numval, (long) slplen);
              }
              numdsp++;
              if (vnp != NULL) {
                vnp = vnp->next;
      ...
      for (vnp = head, numsgp = 0;
           vnp != NULL; vnp = vnp->next, numsgp++) continue;
      if (numdsp != numsgp) {
        gcp->itemID = firstsgitemid;
        ValidErr (vsp, SEV_ERROR, ERR_SEQ_GRAPH_GraphDiffNumber, "Different number of SeqGraph (%d) and SeqLit (%d) components",
                  (int) numsgp, (int) numdsp);
      }
    }
  }
  ...
  for (vnp = head; vnp != NULL; vnp = vnp->next) {
    ...
    if (valsBelowMin > 0) {
      ValidErr (vsp, SEV_WARNING, ERR_SEQ_GRAPH_GraphBelow, "%ld quality scores have values below the reported minimum",
                (long) valsBelowMin);
    }
    if (valsAboveMax > 0) {
      ValidErr (vsp, SEV_WARNING, ERR_SEQ_GRAPH_GraphAbove, "%ld quality scores have values above the reported maximum",
                (long) valsAboveMax);
    }
