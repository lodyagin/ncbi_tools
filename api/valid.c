...

{
  ...
  for (sip = bsp->id; sip != NULL; sip = sip->next) {
    if (sip->choice != SEQID_OTHER) break;
    tsip = (TextSeqIdPtr) sip->data.ptrvalue;
    if (tsip == NULL || tsip->accession == NULL) break;
...
static void ValidateBioseqInst (GatherContextPtr gcp)
{
    ...
    case SEQID_GENERAL:
      ...
      if (dbt != NULL) {
        if (StringICmp (dbt->db, "BankIt") == 0) {
          is_tpa = TRUE; /* this may be a tpa record */
        }
      }
      break;
    default:
  ...
  if (gi_count > 0 && accn_count == 0 && (! isPDB)) {
...
static Boolean IS_NC_IN_SEP (SeqEntryPtr sep)
        ...
        case SEQID_TPD :
          ...
          if (tsip != NULL) {
            if (tsip->accession == NULL && (! StringHasNoText (tsip->name))) {
              ValidErr (vsp, SEV_WARNING, ERR_SEQ_FEAT_BadProductSeqId, "Feature product should have accession");
            }
          }
          break;
        default :
...
NLM_EXTERN void MrnaTransCheck (ValidStructPtr vsp, SeqFeatPtr sfp)
{
  ...
  if (sfp->excpt && (! StringHasNoText (sfp->except_text))) {
    for (i = 0; bypass_mrna_trans_check [i] != NULL; i++) {
      if (StringISearch (sfp->except_text,  bypass_mrna_trans_check [i]) != NULL) return; /* biological exception */
    }
  }
...
NLM_EXTERN void CdTransCheck (ValidStructPtr vsp, SeqFeatPtr sfp)
{
  ...
  if (sfp->excpt && (! StringHasNoText (sfp->except_text))) {
    for (i = 0; bypass_cds_trans_check [i] != NULL; i++) {
      if (StringISearch (sfp->except_text,  bypass_cds_trans_check [i]) != NULL) return; /* biological exception */
    }
  }
